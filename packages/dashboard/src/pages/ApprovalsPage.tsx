  ...
  if (!selectedCompany) {
    return (
      <div className="rounded-xl border border-dashed p-8 text-sm text-muted-foreground">
        Choose a company to review approvals.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Pending approvals for {selectedCompany.name}
        </p>
      </div>
      ...
      <div className="space-y-4">
        {approvals
          ...
          .map((approval) => (
            <div
              key={approval.id}
              className="p-6 bg-yellow-50 border-2 border-yellow-200 rounded-xl shadow-sm flex items-start justify-between gap-6 hover:shadow-md transition-shadow"
            >
