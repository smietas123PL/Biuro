...
describe('GoalsPage', () => {
  ...
  it('generates and applies an AI goal decomposition', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Describe the mission in plain language'), {
      target: { value: 'launch our partner program in Q2 and keep ownership clear' },
    });
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('Starter: Define the launch scope')).toBeTruthy();
    ...
    fireEvent.change(screen.getByDisplayValue('Starter: Define the launch scope'), {
    ...
    await waitFor(() => {
      expect(screen.getByText('Applied AI decomposition with 2 goals and 1 starter task.')).toBeTruthy();
    });
  });
});
