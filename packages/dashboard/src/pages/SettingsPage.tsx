...
function runtimeTone(runtime: RuntimeName) {
  ...
  if (runtime === 'claude') return 'bg-orange-100 text-orange-700 border-orange-200';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    async function loadRuntimeSettings() {
      ...
      try {
        const data = (await request(`/companies/${selectedCompanyId}/runtime-settings`, undefined, { suppressError: true })) as CompanyRuntimeSettingsResponse;
        if (cancelled) {
          return;
        }
  ...
  const handleSave = async () => {
    ...
    setSaveMessage(null);
    try {
      const data = (await request(`/companies/${selectedCompanyId}/runtime-settings`, {
        method: 'PATCH',
        body: JSON.stringify({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Current session, company context, and runtime routing defaults.</p>
      </div>
      ...
      <section className="rounded-2xl border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Choose the primary runtime for this company and reorder the fallback chain used by the multi-provider router.
            </p>
          </div>
        ...
        {!selectedCompanyId ? (
          ...
          <div className="mt-4 text-sm text-muted-foreground">Loading runtime settings...</div>
        ) : !runtimeSettings ? (
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_1.4fr]">
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  This provider is attempted first for company heartbeats before any fallbacks.
                </div>
              </div>
                ...
                {runtimeSettings.available_runtimes.map((runtime) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      draftPrimary === runtime ? 'border-primary bg-primary/5' : 'hover:bg-accent'
                    )}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="font-medium">{runtimeLabel(runtime)}</div>
                      <span className={clsx('inline-flex rounded-full border px-2 py-1 text-xs font-medium', runtimeTone(runtime))}>
                        {draftPrimary === runtime ? 'Primary' : 'Available'}
                      </span>
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      {runtime === 'gemini' && 'Fast default path for most workstreams.'}
                      {runtime === 'claude' && 'Strong fallback for nuanced reasoning and long-form tasks.'}
                      {runtime === 'openai' && 'Balanced fallback for general-purpose execution.'}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Reorder the provider chain used when the primary runtime errors, times out, or rate limits.
                </div>
              </div>

              <div className="space-y-3">
                {draftFallback.map((runtime, index) => (
                  <div key={runtime} className="flex items-center justify-between gap-3 rounded-xl border bg-muted/20 px-4 py-3">
              ...
              <div className="rounded-xl border border-dashed bg-muted/10 p-4 text-sm">
                ...
                <div className="mt-1 text-muted-foreground">
                  Primary: {runtimeLabel(runtimeSettings.system_defaults.primary_runtime)}. Fallbacks: {runtimeSettings.system_defaults.fallback_order.map(runtimeLabel).join(' -> ')}.
                </div>
              </div>
