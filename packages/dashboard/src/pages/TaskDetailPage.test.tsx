...
describe('TaskDetailPage', () => {
        ...
        return {
          ...
          timeline: [
            ...
            },
            {
              ...
              to_agent_name: null,
              to_agent_role: null,
              content: 'Tool Result (web_search): {\"query\":\"Europe pricing\"}',
  ...
  it('renders task force mode with live co-reasoning and delegated workstreams', async () => {
    ...
    expect(screen.getAllByText('Delegation diff').length).toBeGreaterThanOrEqual(2);
    expect(screen.getAllByText('Assigned owner').length).toBeGreaterThanOrEqual(2);
    expect(screen.getAllByText('Fast handoff').length).toBeGreaterThanOrEqual(2);
    expect(screen.getAllByText('Stuck').length).toBeGreaterThanOrEqual(2);
    expect(screen.getByText('Child task picked up within the first minute.')).toBeTruthy();
    expect(screen.getByText('Delegation health: Child task picked up within the first minute.')).toBeTruthy();
    expect(screen.getAllByText('First visible move').length).toBeGreaterThanOrEqual(2);
    expect(screen.getAllByText('Completed in').length).toBeGreaterThanOrEqual(2);
    expect(screen.getByText('10s')).toBeTruthy();
    expect(screen.getByText('3m')).toBeTruthy();
    expect(
      screen
        .getAllByRole('link', { name: 'Open delegated task' })
        .some((link) => link.getAttribute('href')?.includes('/tasks/task-child'))
    ).toBe(true);
  ...
  it('shows replay fork provenance for forked task branches', async () => {
    ...
    expect(screen.getByRole('link', { name: 'source task' }).getAttribute('href')).toBe('/tasks/task-source');
    expect(screen.getByRole('link', { name: 'Open source replay' }).getAttribute('href')).toBe(
      '/agents/agent-7?task_id=task-source&event_id=heartbeat%3Aheartbeat-7'
    );
    expect(screen.getByText('Replay event: heartbeat:heartbeat-7')).toBeTruthy();
  ...
  it('filters the timeline by event type and expands grouped tool activity', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Tool called web_search with Europe pricing query.')).toBeTruthy();
      expect(screen.getByText('Tool Result (web_search): {"query":"Europe pricing"}')).toBeTruthy();
    });
  });
  ...
  it('filters Task Force Map to risky handoffs only', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Show only risky handoffs (1)' })).toBeTruthy();
    });

    fireEvent.click(screen.getByRole('button', { name: 'Show only risky handoffs (1)' }));

    await waitFor(() => {
      const taskForceMapSection = screen.getByText('Task Force Map').closest('section');
      if (!taskForceMapSection) {
        throw new Error('Expected Task Force Map section');
      }

      const taskForceMap = within(taskForceMapSection);
      expect(screen.getByRole('button', { name: 'Show all workstreams' })).toBeTruthy();
      expect(taskForceMap.getAllByText('Ship launch plan').length).toBeGreaterThanOrEqual(1);
      expect(taskForceMap.queryByText('Delegated: Validate messaging')).toBeNull();
      expect(taskForceMap.getAllByText('Stuck').length).toBeGreaterThanOrEqual(1);
      expect(taskForceMap.getByText('Delegation health: Delegated child task has no visible follow-up activity yet.')).toBeTruthy();
    });
  });
  ...
  it('filters Task Force Map from summary facets', async () => {
    ...
    await waitFor(() => {
      ...
      expect(taskForceMap.getAllByText('Stuck').length).toBeGreaterThanOrEqual(1);
      expect(taskForceMap.queryByText('Delegated: Validate messaging')).toBeNull();
      expect(taskForceMap.getByRole('button', { name: 'Clear facet' })).toBeTruthy();
      expect(taskForceMap.getByText('Delegation health: Delegated child task has no visible follow-up activity yet.')).toBeTruthy();
    });
  });
  ...
  it('restores Task Force Map facet from the URL', async () => {
    render(
      <MemoryRouter initialEntries={['/tasks/task-root?taskMapFilter=stuck']} future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
        <Routes>
          <Route path="/tasks/:id" element={<TaskDetailPage />} />
        </Routes>
      </MemoryRouter>
  ...
  it('persists timeline filter and expanded tool sequence in the URL', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('location-search').textContent).toContain('timelineFilter=tool');
      expect(screen.getByText('Tool sequence')).toBeTruthy();
    });
  ...
  it('restores timeline filter and expanded tool sequence from the URL', async () => {
    render(
      <MemoryRouter
        initialEntries={['/tasks/task-root?timelineFilter=tool&expandedTools=2026-03-18T22%3A02%3A00.000Z-task-child-tool-1-tool-2']}
  ...
  it('persists the active page mode in the URL', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('location-search').textContent).toContain('view=overview');
    });
  ...
  it('persists the focused Task Force Map workstream in the URL', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByRole('button', { name: 'Focus' }).length).toBeGreaterThan(0);
    });
    ...
    await waitFor(() => {
      ...
      expect(taskForceMap.getAllByText('Delegated: Validate messaging').length).toBeGreaterThanOrEqual(1);
      expect(taskForceMap.getByRole('button', { name: 'Focused' })).toBeTruthy();
      expect(screen.getByTestId('location-search').textContent).toContain('taskFocus=task-child');
    });
  ...
  it('restores the focused Task Force Map workstream from the URL', async () => {
    ...
    await waitFor(() => {
      ...
      const taskForceMap = within(taskForceMapSection);
      expect(taskForceMap.getByText('Map focus')).toBeTruthy();
      expect(taskForceMap.getAllByText('Delegated: Pricing audit').length).toBeGreaterThanOrEqual(1);
      expect(taskForceMap.getByRole('button', { name: 'Focused' })).toBeTruthy();
    });
  });
});
