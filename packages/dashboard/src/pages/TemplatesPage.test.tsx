...
describe('TemplatesPage', () => {
  beforeEach(() => {
    ...
    requestMock.mockImplementation(async (path: string, options?: RequestInit) => {
      ...
      if (path === '/agents?company_id=company-1') {
        return [
          { id: 'agent-1', name: 'Riley', role: 'ops_lead', title: 'Ops Lead', status: 'working' },
          { id: 'agent-2', name: 'Mina', role: 'researcher', title: 'Research Analyst', status: 'idle' },
        ];
      }
      ...
      if (path === '/templates/import-marketplace/market-1' && options?.method === 'POST') {
        return {
      ...
      if (path === '/templates/ai-suggest' && options?.method === 'POST') {
        return {
          suggestion: {
            ...
            description: 'Inspect recent competitor pricing updates, summarize the main deltas, and flag anything that needs a response from the team.',
            priority: 72,
  ...
  it('loads marketplace templates and installs them through the dry-run flow', async () => {
    ...
    await waitFor(() => {
      ...
      expect(requestMock).toHaveBeenCalledWith('/agents?company_id=company-1', undefined, {
        suppressError: true,
      });
    });
  ...
  it('generates an AI draft and creates a task from it', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Describe the work in plain language'), {
      target: { value: 'sprawdz czy konkurencja obniżyła ceny i przygotuj krótkie podsumowanie' },
    });
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('Review competitor pricing changes')).toBeTruthy();
      expect(screen.getByText('Planned by claude')).toBeTruthy();
    });

    fireEvent.click(screen.getByRole('button', { name: 'Create Task From Draft' }));
    ...
    expect(screen.getByText('Created task "Review competitor pricing changes" from AI draft.')).toBeTruthy();
  });
});
