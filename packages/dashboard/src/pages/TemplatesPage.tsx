...
export default function TemplatesPage() {
  ...
  const [catalog, setCatalog] = useState<MarketplaceResponse['catalog'] | null>(
    null
  );
  ...
  const [aiPlanner, setAiPlanner] = useState<
    AISuggestResponse['planner'] | null
  >(null);
  ...
  useEffect(() => {
    const load = async () => {
      const [local, marketplace, agents] = await Promise.all([
        request('/templates/presets') as Promise<
          Array<Omit<Summary, 'source'>>
        >,
        request('/templates/marketplace') as Promise<MarketplaceResponse>,
        selectedCompanyId
          ? (request(`/agents?company_id=${selectedCompanyId}`, undefined, {
              suppressError: true,
      ...
      const normalizedMarket = marketplace.templates.map((item) => ({
        ...item,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [nextDetail, preview] = await Promise.all([
        ...
        selectedCompanyId
          ? (request(ep.dryRun) as Promise<{ preview: DryRun }>)
  ...
  if (!selectedCompany) {
    return (
      <div className="rounded-xl border border-dashed p-8 text-sm text-muted-foreground">
        Choose a company to explore templates.
      </div>
    );
  }

  const canInstall =
    confirmationText.trim().toUpperCase() === 'IMPORT' && Boolean(dryRun);
  ...
  const install = async () => {
    ...
    try {
      ...
      setConfirmationText('');
      setSuccessMessage(
        `Installed "${result.template?.name ?? result.preset?.name}" into ${selectedCompany.name}.`
      );
    } finally {
      setBusy(false);
    }
  ...
  const savePreview = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Saved preview snapshot for "${result.template?.name ?? result.preset?.name}".`
      );
    } finally {
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Local presets plus an installable marketplace for{' '}
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Describe a task in natural language and get an editable draft with
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="text-xs text-muted-foreground">
                Uses company runtime routing first, then falls back to a
                deterministic draft if needed.
              </div>
            </div>
          </div>
            ...
            {aiDraft ? (
              <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(event) => {
                          ...
                          const nextAgent =
                            agentOptions.find((agent) => agent.id === nextId) ??
                            null;
                          setAiDraft((current) =>
                            current
                              ? {
                                  ...
                                  default_role:
                                    nextAgent?.role ?? current.default_role,
                                }
                              : current
                          );
                        }}
                ...
                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  ...
                  {aiDraft.suggested_agent_name ? (
                    <Chip>Agent: {aiDraft.suggested_agent_name}</Chip>
                  ) : null}
                </div>
              ...
              <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">
                No AI draft yet. Describe the work on the left to generate a
                pre-filled task draft.
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_1.45fr]">
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          <div className="mb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Switch between local presets and the external marketplace catalog.
            </p>
          </div>
          <div className="mb-4 grid gap-3 sm:grid-cols-2">
            <SourceCard
              icon={Store}
              active={source === 'marketplace'}
              title="Marketplace"
              body="External catalog + bundled fallback."
              onClick={() => {
                setSource('marketplace');
                setSelectedId(marketItems[0]?.id ?? null);
              }}
            />
            <SourceCard
              icon={Layers3}
              active={source === 'local'}
              title="Local Library"
              body="Curated templates bundled with Biuro."
              onClick={() => {
                setSource('local');
          ...
          <div className="space-y-3">
            ...
            {activeItems.length === 0 && !loading && (
              <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">
                No templates available in this library.
              </div>
            )}
          </div>
        </div>
          ...
          {detail && activeSummary ? (
            <div className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Block
                  title="Goals"
                  items={detail.template.goals.map((goal) => goal.title)}
                />
                <Block
                  title="Agents"
                  items={detail.template.agents.map(
                    (agent) => `${agent.name} - ${agent.title || agent.role}`
                  )}
                />
                <Block
                  title="Tools"
                  items={detail.template.tools.map(
                    (tool) => `${tool.name} - ${tool.type}`
                  )}
                />
                <Block
                  title="Policies"
                  items={detail.template.policies.map(
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          Review what will be added or updated in{' '}
                          {selectedCompany.name} before installing this
                          template.
                        </div>
                      </div>
                      ...
                      <div className="rounded-2xl border bg-background p-4 text-sm text-muted-foreground">
                        ...
                        {dryRun.company.resulting_mission
                          ? ` - ${dryRun.company.resulting_mission}`
                          : ''}{' '}
                        ·{' '}
                        {dryRun.preserve_company_identity
                          ? 'Identity preserved'
                          : 'Identity replaced'}
                      </div>
                      <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <SimpleList
                          ...
                          empty="No existing tools will be updated"
                        />
                        <SimpleList
                          title="Warnings"
                          items={dryRun.warnings}
                      ...
                      {dryRun.collisions.agent_names.length +
                        ...
                        <div className="rounded-2xl border bg-amber-50/70 p-4 text-sm text-amber-900">
                          <div className="flex items-center gap-2 font-medium">
                            ...
                            Overlap signals detected
                          </div>
                          <div className="mt-2 text-xs">
                            Agents: {dryRun.collisions.agent_names.length},
                            goals: {dryRun.collisions.goal_titles.length},
                            policies: {dryRun.collisions.policy_names.length},
                            tools: {dryRun.collisions.tool_names.length}
                          </div>
                        </div>
                      )}
                      <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          >
                            {busy
                              ? 'Installing...'
                              : `Install Into ${selectedCompany.name}`}
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">
              Select a template to inspect its structure.
            </div>
          )}
        </div>
      </div>
