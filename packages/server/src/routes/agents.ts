...
const replayForkSchema = z.object({
  ...
  task_id: z.string().min(1).optional(),
  types: z
    .array(z.enum(['heartbeat', 'audit', 'message', 'session']))
    .optional(),
...
function getFailureSignal(details?: Record<string, unknown>) {
  ...
  if (
    ...
    Array.isArray((routing as { attempts?: unknown[] }).attempts)
  ) {
    const attempts = (
      (routing as { attempts?: unknown[] }).attempts ?? []
    ).filter(
      (
        attempt
      ): attempt is {
    ...
    if (
      ...
      return failedAttempts
        .map((attempt) => {
          const runtime =
            typeof attempt.runtime === 'string' ? attempt.runtime : 'runtime';
          const model =
            typeof attempt.model === 'string' ? attempt.model : 'model';
          const reason =
            typeof attempt.reason === 'string' ? attempt.reason : 'failed';
...
function classifyFailureSeverity(item: ReplayEvent): 'high' | 'medium' | 'low' {
  ...
  if (
    status === 'blocked' ||
    status === 'budget_exceeded' ||
    /(blocked|budget_exceeded)/.test(action)
  ) {
    return 'medium';
  }
  return 'low';
...
function buildFailureEvidence(items: ReplayEvent[], targetIndex: number) {
  ...
  const evidence = [
    ...
    getFailureSignal(target.details)
      ? `Signal: ${getFailureSignal(target.details)}`
      : null,
        ...
        `LLM attempts: ${attempts
          .map((attempt) => {
            ...
            const model =
              typeof attempt.model === 'string' ? attempt.model : 'model';
            const status =
              typeof attempt.status === 'string' ? attempt.status : 'unknown';
            const reason =
...
function buildFailureRecommendations(item: ReplayEvent) {
  ...
  if (
    ...
    signal.includes('http') ||
    signal.includes('web_search')
  ) {
    return [
...
function buildHeuristicFailureExplanation(
  items: ReplayEvent[],
  targetIndex: number
  ...
  return {
    ...
    explanation: {
      ...
      likely_cause:
        signal ??
        'The replay indicates the agent hit an execution error without a cleaner structured reason.',
...
async function generateFailureExplanation(
  ...
  const windowItems = payload.items.slice(
    Math.max(0, targetIndex - 4),
    Math.min(payload.items.length, targetIndex + 3)
  );
  ...
  try {
    ...
    const response = await runtime.execute({
      ...
      current_task: {
        ...
        description:
          'Diagnose the failure, explain it plainly, and recommend the next actions.',
      },
      goal_hierarchy: [],
      additional_context: [
    ...
    const fallback = buildHeuristicFailureExplanation(
      payload.items,
      targetIndex
    );
    return {
...
function findReplaySessionSnapshot(
  items: ReplayEvent[],
  anchorIndex: number,
  taskId: string
) {
...
function buildReplayReportHtml(payload: ReplayPayload) {
  const selectedTypesLabel =
    payload.filters.applied.types.length > 0
      ? payload.filters.applied.types.join(', ')
      : 'all';
  const selectedTaskLabel = payload.filters.applied.task_id
    ? (payload.filters.tasks.find(
        (task) => task.task_id === payload.filters.applied.task_id
      )?.task_title ?? payload.filters.applied.task_id)
    : 'All tasks';
...
async function buildReplayPayload(
  ...
  const [heartbeatsResult, auditResult, messageResult, sessionResult] =
    await Promise.all([
      db.query(
  ...
  const allItems: ReplayEvent[] = [
    ...heartbeatsResult.rows.map(
      (row): ReplayEvent => ({
        id: `heartbeat:${row.id}`,
        type: 'heartbeat',
        action: `heartbeat.${row.status}`,
        timestamp: String(row.created_at),
        summary: formatReplaySummary(
          row.details?.thought,
          row.task_title
            ? `Heartbeat on ${row.task_title}`
            : `Heartbeat marked ${row.status}`
        ),
        task_id: row.task_id,
    ...
    ...auditResult.rows.map(
      (row): ReplayEvent => ({
        ...
        timestamp: String(row.created_at),
        summary: formatReplaySummary(
          row.details?.reason ?? row.details?.message,
    ...
    ...messageResult.rows.map(
      (row): ReplayEvent => ({
        ...
        type: 'message',
        action:
          row.from_agent === agentId ? 'message.sent' : 'message.received',
        timestamp: String(row.created_at),
        summary: formatReplaySummary(
          row.content,
          row.from_agent === agentId ? 'Sent a message' : 'Received a message'
        ),
        task_id: row.task_id,
        task_title: row.task_title,
    ...
    ...sessionResult.rows.map(
      (row): ReplayEvent => ({
        ...
        summary: formatReplaySummary(
          row.state?.summary,
          row.task_title
            ? `Session updated for ${row.task_title}`
  ...
  async (req, res, next) => {
    try {
      ...
      if (left_task_id === right_task_id) {
        return res
          .status(400)
          .json({ error: 'Choose two different tasks to compare' });
      }
      ...
      const response: ReplayDiffPayload = {
        ...
        delta: {
          ...
          total_cost_usd: Number(
            (left.total_cost_usd - right.total_cost_usd).toFixed(4)
          ),
        },
      };
  ...
  async (req, res, next) => {
    try {
      ...
      const targetIndex = parsed.data.event_id
        ? replayPayload.items.findIndex(
            (item) => item.id === parsed.data.event_id
          )
        : ([...replayPayload.items]
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const anchorIndex = replayPayload.items.findIndex(
        (item) => item.id === parsed.data.replay_event_id
      );
      if (anchorIndex === -1) {
        return res.status(404).json({
          error: 'Replay event not found in the current session scope',
        });
      }
      ...
      if (!anchorEvent.task_id) {
        return res.status(400).json({
          error:
            'This replay event is not tied to a task, so it cannot be forked.',
        });
      }
      ...
      const taskTitle = buildForkTaskTitle(
        sourceTask.title,
        parsed.data.fork_title
      );
      const taskDescription = [
        ...
        promptOverride
          ? 'Includes a supervisor prompt override for the rerun branch.'
          : '',
      ]
        .filter(Boolean)
        .join('\n\n');
      ...
      const forkTask = forkTaskResult.rows[0] as {
        id: string;
        title: string;
        company_id: string;
      };
      ...
      const sessionSnapshot = findReplaySessionSnapshot(
        replayPayload.items,
        anchorIndex,
        sourceTask.id
      );
