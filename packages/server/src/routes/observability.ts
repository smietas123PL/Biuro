...
import { getRecentSpans, getTracingServiceName } from '../observability/tracing.js';
...
router.get('/traces/recent', requireRole(['owner', 'admin']), async (req, res) => {
...
router.get('/traces/:traceId', requireRole(['owner', 'admin']), async (req, res) => {
  ...
  const matchingSpans = getRecentSpans(200)
    ...
    .sort((left, right) => Date.parse(left.start_time) - Date.parse(right.start_time)) as ObservabilitySpanItem[];

  if (matchingSpans.length === 0) {
    return res.status(404).json({ error: 'Trace not found in recent history' });
  }

  const startedAt = matchingSpans[0]?.start_time ?? new Date().toISOString();
  const endedAt = matchingSpans[matchingSpans.length - 1]?.end_time ?? startedAt;
  const durationMs = Number(
    matchingSpans.reduce((longest, span) => Math.max(longest, span.duration_ms), 0).toFixed(3)
  );
