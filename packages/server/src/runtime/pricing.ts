...
function loadPricingOverrides(): PricingOverrideMap {
  ...
  try {
    ...
    const normalized = Object.entries(parsed).reduce<PricingOverrideMap>(
      (acc, [key, value]) => {
    ...
    logger.warn(
      { err, raw },
      'Failed to parse LLM_PRICING_OVERRIDES, falling back to default runtime pricing'
    );
    cachedOverrideRaw = raw;
    cachedOverrides = {};
    return cachedOverrides;
...
function resolveModelPricing(
  ...
  if (model) {
    ...
    const wildcardEntry = Object.entries(overrides).find(
