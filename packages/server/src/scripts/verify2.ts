...
async function verifyPhase2() {
  ...
  try {
    ...
    const companyRes = await db.query(
      "INSERT INTO companies (name, mission) VALUES ('Safety Test Corp', 'Safety First') RETURNING id"
    );
    ...
    await db.query(
      'INSERT INTO agent_tools (agent_id, tool_id, can_execute) VALUES ($1, $2, true)',
      [agentId, toolId]
    );
    ...
    const policyOk = await evaluatePolicy(companyId, 'budget_threshold', {
      amount: 50,
    });
    console.log('Spend $50:', policyOk.allowed ? 'Allowed' : 'Blocked');

    const policyBlocked = await evaluatePolicy(companyId, 'budget_threshold', {
      amount: 500,
    });
    console.log(
      'Spend $500:',
      policyBlocked.requires_approval ? 'Requires Approval' : 'Denied'
    );

    // 4. Test Safety System (Flood Detection)
    logger.info('Testing Safety System (Flood Detection)...');
    const taskRes = await db.query(
      "INSERT INTO tasks (company_id, title, status) VALUES ($1, 'Spam Task', 'backlog') RETURNING id",
      [companyId]
    );
    ...
    for (let i = 0; i < 12; i++) {
      await db.query(
        "INSERT INTO messages (company_id, task_id, from_agent, content) VALUES ($1, $2, $3, 'Spam')",
    ...
    console.log(
      'Safety Check after spam:',
      safetyCheck.ok ? 'OK' : `FAIL: ${safetyCheck.reason}`
    );

    if (!safetyCheck.ok) {
      await autoPauseAgent(agentId, safetyCheck.reason!);
      const agentStatus = await db.query(
        'SELECT status, config FROM agents WHERE id = $1',
        [agentId]
      );
      console.log(
        'Agent status after auto-pause:',
        agentStatus.rows[0].status,
        agentStatus.rows[0].config.pause_reason
      );
    }

    // 5. Test Approval Workflow
    logger.info('Testing Approval Workflow...');
    const appReq = await createApprovalRequest(
      companyId,
      taskId,
      agentId,
      'Need to buy coffee',
