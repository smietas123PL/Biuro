...
async function verifyPhase3() {
  ...
  try {
    ...
    const companyRes = await db.query(
      "INSERT INTO companies (name, mission) VALUES ('Production Test', 'Test everything') RETURNING id"
    );
    ...
    try {
      ...
      console.log(
        'UNAUTHORIZED check passed (401 expected):',
        err.response?.status
      );
    }
    ...
    const agentId = (
      await db.query(
        "INSERT INTO agents (company_id, name, role, runtime) VALUES ($1, 'MemAgent', 'Thinker', 'openai') RETURNING id",
        [companyId]
      )
    ).rows[0].id;
    ...
    await storeMemory(
      ...
      agentId,
      'task-1',
      'This is a test experience about coding.'
    );
    ...
    logger.error(
      { err: err.message, data: err.response?.data },
