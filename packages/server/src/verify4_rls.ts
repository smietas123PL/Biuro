...
async function verifyIsolation() {
  ...
  try {
    ...
    await db.query("INSERT INTO companies (id, name, mission) VALUES ($1, 'Company Alpha', 'Mission A')", [c1Id]);
    await db.query("INSERT INTO companies (id, name, mission) VALUES ($1, 'Company Beta', 'Mission B')", [c2Id]);

    // 2. Insert data into Company Alpha
    await db.query("INSERT INTO agents (company_id, name, role, runtime) VALUES ($1, 'Agent Alpha', 'Sec-Ops', 'openai')", [c1Id]);
    ...
    await contextStore.run({ companyId: c2Id }, async () => {
      ...
      console.log(`Company Beta sees ${agents.rows.length} agents (Expected: 0)`);
    ...
    await contextStore.run({ companyId: c1Id }, async () => {
      const agents = await db.query("SELECT * FROM agents");
      console.log(`Company Alpha sees ${agents.rows.length} agents (Expected: 1)`);
      if (agents.rows.length !== 1) throw new Error("RLS NOT WORKING AS EXPECTED!");
    });
