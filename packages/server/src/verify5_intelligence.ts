...
async function runVerification() {
  ...
  await db.query("INSERT INTO companies (id, name) VALUES ($1, 'Test Corp') ON CONFLICT (id) DO NOTHING", [companyId]);
  await db.query("INSERT INTO agents (id, company_id, name, role) VALUES ($1, $2, 'Intelligence Agent', 'Researcher') ON CONFLICT (id) DO NOTHING", [agentId, companyId]);
  ...
  if (context.knowledge_context?.includes('Antigravity2024')) {
    console.log('✅ Context injection successful: Secret password found in context!');
  } else {
  ...
  const slackResult = await IntegrationService.handleSlashCommand('/biuro-task', 'Analyze Q4 revenue patterns', companyId);
  ...
  try {
     ...
     if (typeof MCPService.callTool === 'function') console.log('✅ MCPService available');
  } catch (e) {
